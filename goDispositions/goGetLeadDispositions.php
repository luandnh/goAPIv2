...
$lead_id                                            = (isset($_REQUEST["lead_id"])) ? $_REQUEST['lead_id'] : null;
...
if (empty($goUser) || is_null($goUser)) {
	...
	if ($goapiaccess > 0 && $userlevel > 7) {
		if (is_array($campaigns)) {
			...
			$lead_info = $astDB->getOne("vicidial_list",array("list_id"));

			$astDB->where("list_id",$lead_info['list_id']);
			$list_info = $astDB->getOne("vicidial_lists",array("campaign_id"));
			...
			$astDB->where("campaign_id",$list_info['campaign_id']);
