...
$file_name = "Export_Call_Log_" . $fromDate ."-".$toDate.".csv";
$file_path = SAVE_FOLDER . $file_name;
$time_pre = microtime(true);
$init_download = array(
    'name' => 'EXPORT_CALLREPORT',
    'filename' => $file_name,
    'username' => $goUser,
    'path' => $file_path,
...
if ($RUNcampaign > 0 && $RUNgroup < 1) {
    ...
    $user_group_SQL $list_SQL $status_SQL_vl
...
while ($tmp != 0 || $running_time > 60 * 12) {
    $running_time = microtime(true) - $time_pre;
    $csv_row = array();
    $batch_stmt = $query  . " LIMIT " . LIMIT . ' OFFSET ' . $offset;
    go_logger($batch_stmt,"QUANGBUG.log");
    $result = $astDB->rawQuery($batch_stmt);
    $tmp = count($result);
    go_logger($tmp,"QUANGBUG.log");
    if ($tmp == 0){break;}
    //OUTPUT DATA HEADER//
    ...
    foreach ($result as $row) {
        ...
        try {
            ...
            go_logger($th->getMessage(),"QUANGBUG.log");
        }
        fputcsv($file, $row);
    }
