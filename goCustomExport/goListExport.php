	...
	try {
		...
		if ($goapiaccess > 0 && $userlevel > 7) {
			...
			if ($added_custom_SQL3 != "") {
				...
				(SELECT val.event_time from vicidial_agent_log val WHERE val.lead_id = vl.lead_id limit 1) as first_call ,vl.status,vs.status_name,user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,vl.phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,vl.province,postal_code,country_code,gender,vl.identity_number,vl.alt_identity_number,vl.date_of_birth,alt_phone,email,security_phrase,comments,called_count,date_format(last_local_call_time, '%Y-%m-%d %H:%i:%s') as last_local_call_time,rank,owner {$header_columns} $collabrator_col_SQL FROM vicidial_list vl LEFT OUTER JOIN {$added_custom_SQL3} ON {$added_custom_SQL4} $collabrator_SQL
				...
				(SELECT val.event_time from vicidial_agent_log val WHERE val.lead_id = vl.lead_id limit 1) as first_call,vl.status,vs.status_name, user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,vl.phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,vl.province,postal_code,country_code,gender,vl.identity_number,vl.alt_identity_number,vl.date_of_birth,alt_phone,email,security_phrase,comments,called_count,date_format(last_local_call_time, '%Y-%m-%d %H:%i:%s') as last_local_call_time,rank,owner FROM vicidial_list vl 
				LEFT OUTER JOIN vicidial_statuses vs ON vl.status = vs.status
