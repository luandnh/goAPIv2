...
if ($sipIsLoggedIn) {
    ...
    if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) ) {
        ...
        if ($affected_rows > 0 && $errmsg < 1) {
            ...
            $astDB->where('lead_id', $lead_id);
            //EASYCREDIT
            $rslt = $astDB->getOne('vicidial_list', 'lead_id,entry_date,modify_date,status,user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,province,postal_code,country_code,gender,date_of_birth,alt_phone,email,security_phrase,comments,called_count,last_local_call_time,rank,owner,entry_list_id,partner_code,app_status,identity_number,identity_issued_on,identity_issued_by,alt_identity_number,alt_identity_issued_on,alt_identity_issued_by,request_id,call_status,reject_reason');
            //EASYCREDIT
            ...
            #	$lead_id		= trim("$row['lead_id']");
                ...
                $alt_identity_number = trim("{$row['alt_identity_number']}");
                $alt_identity_issued_on = trim("{$row['alt_identity_issued_on']}");
                $alt_identity_issued_by = trim("{$row['alt_identity_issued_by']}");
    ...
    #				$dial_channel = "$local_DEF$dial_ingroup_dialstring$local_AMP$ext_context$Local_persist";
            ...
            if (strlen($list_id) > 0) {
                ...
                //EASYCREDIT
                $stmt = "SELECT field_label,field_type FROM vicidial_lists_fields where list_id='$entry_list_id' and field_type NOT IN('DISPLAY') and field_label NOT IN('vendor_lead_code','source_id','list_id','gmt_offset_now','called_since_last_reset','phone_code','phone_number','title','first_name','middle_initial','last_name','address1','address2','address3','city','state','province','postal_code','country_code','gender','date_of_birth','alt_phone','email','security_phrase','comments','called_count','last_local_call_time','rank','owner','partner_code','request_id');";
