...
$offset												= (isset($_REQUEST["offset"]) && !empty($_REQUEST["offset"])) ? $astDB->escape($_REQUEST["offset"]) : 0;
$limit 												= (isset($_REQUEST["limit"]) && !empty($_REQUEST["limit"])) ? $astDB->escape($_REQUEST["limit"]) : 100;
$list_ids											= array();
...
if (empty($goUser) || is_null($goUser)) {
	...
	if ($goapiaccess > 0 && $userlevel >= 7) {
		...
		if (!empty($identity_filter)) {
			$astDB->where("identity_number", "%$identity_filter%", "LIKE");
		}
		if (!empty($leadcode_filter)) {
			$astDB->where("vendor_lead_code", "%$leadcode_filter%", "LIKE");
		}
		if (!empty($leadsubid_filter)) {
			$astDB->where("lead_sub_id", "%$leadsubid_filter%", "LIKE");
		}

		$astDB->where("list_id", $list_ids, "IN");
		$astDB->orderBy('lead_id', 'asc');
		$fresultsv 									= $astDB->get("vicidial_list", array($offset, $limit), "*", true);
