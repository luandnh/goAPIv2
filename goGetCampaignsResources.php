...
*/
	...
	if (empty($goUser) || is_null($goUser)) {
		...
		$data = $astDB->where('user_group', $log_group);
		$allowed_camps = $data->getOne('vicidial_user_groups', 'allowed_campaigns');
		...
		if ($goapiaccess > 0 && $userlevel >= 7) {
			...
			if($userlevel >= 7) {
				if (!preg_match("/ALL-CAMPAIGN/", $allowed_campaigns)) {
					...
					$astDB->orWhere('vl.campaign_id', $allowed_campaigns, 'in');
				}
			}
