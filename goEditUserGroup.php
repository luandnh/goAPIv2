...
$manage_usergroups									= $_REQUEST['manage_usergroups'];
	...
	if ($goapiaccess > 0 && $userlevel > 7) {
		...
		if ($astDB->count > 0) {
			...
			if ($goDB->count > 0) {
				...
				$astDB->where('user_group', $user_group);
				$delete_status = $astDB->delete('vicidial_sub_user_groups', null);
				$manage_usergroups = explode(",", $manage_usergroups);
				foreach ($manage_usergroups as $group) {
					$tmp_group = $goDB->where("user_group", $group)->getOne("user_access_group", "user_group, group_level");
					if (isset($tmp_group) && $tmp_group['group_level'] <= 7) {
						$insertData = array(
							'user_group' => $user_group,
							'sub_user_group' => $group,
						);
						$astDB->insert('vicidial_sub_user_groups', $insertData);
					}
					unset($tmp_group);
				}
			}
