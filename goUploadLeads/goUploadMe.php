...
$lead_mapping = NULL;
if (!empty($_REQUEST["lead_mapping"])) $lead_mapping = $astDB->escape($_REQUEST["lead_mapping"]);
...
if (($handle = fopen($csv_file, "r")) !== false)
{
    ...
    while (($data = fgetcsv($handle, 1000, $default_delimiter)) !== false)
    {
        ...
        $gmt_offset = "0";
        // PHONE CODE OVERRIDE
        if (!empty($phone_code_override)) $phone_code = preg_replace($field_regx, "", $phone_code_override);
        else $phone_code = preg_replace($field_regx, "", $col[0]);

        $phone_number = preg_replace($field_regx, "", $col[1]);
        $vendor_lead_code = preg_replace($field_regx, "", $col[2]);
        // $title = preg_replace($field_regx, "", $col[3]);
        $first_name = preg_replace($field_regx, "", $col[3]);
        $middle_initial = preg_replace($field_regx, "", $col[4]);
        $last_name = preg_replace($field_regx, "", $col[5]);
        $address1 = preg_replace($field_regx, "", $col[6]);
        $address2 = preg_replace($field_regx, "", $col[7]);
        $address3 = preg_replace($field_regx, "", $col[8]);
        $city = preg_replace($field_regx, "", $col[9]);
        // $state = preg_replace($field_regx, "", $col[11]);
        $province = preg_replace($field_regx, "", $col[10]);
        $postal_code = preg_replace($field_regx, "", $col[11]);
        $country_code = preg_replace($field_regx, "", $col[12]);
        $gender = preg_replace($field_regx, "", $col[13]);
        $date_of_birth = preg_replace($field_regx, "", $col[14]);
        $date_of_birth = date("Y-m-d", strtotime($date_of_birth));
        $alt_phone = preg_replace($field_regx, "", $col[15]);
        $email = preg_replace($field_regx, "", $col[16]);
        $security_phrase = preg_replace($field_regx, "", $col[17]);
        $comments = preg_replace($field_regx, "", $col[18]);
        $identity_number = preg_replace($field_regx, "", $col[19]);
        ...
        if (!empty($lead_mapping))
        {
            ...
            {
                if (in_array($lead_mapping_fields[$l], $standard_fields)) $standard_array[$lead_mapping_fields[$l]] = $lead_mapping_data[$l];
                else $custom_array[$lead_mapping_fields[$l]] = $lead_mapping_data[$l];
            }
            //set default values to none
            $phone_number = "";
            $vendor_lead_code = "";
            if (!empty($phone_code_override)) $phone_code = $phone_code_override;
            else $phone_code = 1;
            ...
            foreach ($standard_array as $l => $map_data)
            {
                ...
                if ($map_data !== "" || $map_data !== ".")
                {
                    ...
                    if ($l == "Phone") $phone_number = $col[$map_data];
                    if ($l == "LeadCode") $vendor_lead_code = $col[$map_data];
                    if ($l == "PhoneCode")
                    {
                        if (!empty($phone_code_override)) $phone_code = $phone_code_override;
                        else $phone_code = $col[$map_data];
                    }
                    if ($l == "FirstName") $first_name = $col[$map_data];
                    if ($l == "MiddleInitial") $middle_initial = $col[$map_data];
                    if ($l == "LastName") $last_name = $col[$map_data];
                    if ($l == "Address1") $address1 = $col[$map_data];
                    if ($l == "Address2") $address2 = $col[$map_data];
                    if ($l == "Address3") $address3 = $col[$map_data];
                    if ($l == "City") $city = $col[$map_data];
                    if ($l == "Province") $province = $col[$map_data];
                    if ($l == "PostalCode") $postal_code = $col[$map_data];
                    if ($l == "CountryCode") $country_code = $col[$map_data];
                    if ($l == "Gender") $gender = $col[$map_data];
                    if ($l == "DateOfBirth") $date_of_birth = $col[$map_data];
                    if ($l == "AltPhone") $alt_phone = $col[$map_data];
                    if ($l == "Email") $email = $col[$map_data];
                    if ($l == "SecurityPhrase") $security_phrase = $col[$map_data];
                    if ($l == "Comments") $comments = $col[$map_data];
                    if ($l == "IdentityNumber") $identity_number = $col[$map_data];
                    if ($l == "IdentityIssuedOn") $identity_issued_on = $col[$map_data];
                    if ($l == "IdentityIssuedBy") $identity_issued_by = $col[$map_data];
                    if ($l == "PartnerCode") $identity_issued_by = $col[$map_data];
                } // end if
        ...
        if ($goDupcheck === "DUPSYS")
        { // Duplicate check all phone numbers in entire system
    ...
    if ($goCountInsertedLeads > 0 && $duplicates < 1)
    {
        $apiresults = array(
            "result" => "success",
            "message" => "Total Uploaded Leads: $goCountInsertedLeads",
            "alex_data" => $alex
        );
    }
    elseif ($goCountInsertedLeads > 0 && $duplicates > 0)
    {
        $apiresults = array(
            "result" => "success",
            "message" => "Uploaded:$goCountInsertedLeads    Duplicates:$duplicates"
        );
    }
    elseif ($goGetCheckcustomFieldNamesCorrect === "error")
    {
        $apiresults = array(
            "result" => "error",
            "message" => "Error: Lead File Not Compatible with List. Incompatible Field Names. Check the File Headers"
        );
    }
    elseif ($duplicates > 0)
    {
