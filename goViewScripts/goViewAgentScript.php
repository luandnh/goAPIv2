...
if (isset($_GET["partner_code"]))	{$alt_phone=$astDB->escape($_GET["partner_code"]);}
	elseif (isset($_POST["partner_code"]))	{$partner_code=$astDB->escape($_POST["partner_code"]);}
...
if (preg_match("/iframe\ssrc/i",$script_text)) {
	...
	$partner_code = preg_replace('/\s/i','+',$partner_code);
...
$script_text = preg_replace('/--A--partner_code--B--/i',"$partner_code",$script_text);
